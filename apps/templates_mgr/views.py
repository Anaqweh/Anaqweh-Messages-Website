...
def template_list(request):
    return render(request, 'templates_mgr/template_list.html', {'templates': EmailTemplate.objects.all()})
...
def template_create(request):
    if request.method == 'POST':
        ...
        if not name or not subject or not body_html:
            ...
            t = EmailTemplate.objects.create(name=name, subject=subject, body_html=body_html, body_text=body_text)
